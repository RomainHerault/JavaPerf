...
public class CPainting extends Canvas implements MouseListener {
  ...
  // couleur du fond
  private Color mCouleurFond = colorManager.getColor(255, 255, 255);
  // dimensions
    ...
    synchronized (mMutexCouleurs) {
      for (i = 0; i != mDimension.width; i++) {
        for (j = 0; j != mDimension.height; j++) {
          mCouleurs[i][j] = colorManager.getColor(mCouleurFond.getRed(), mCouleurFond.getGreen(), mCouleurFond.getBlue());
        }
      }
    }
        ...
        case 1:
          ...
          for (i = 0; i < 3; i++) {
            for (j = 0; j < 3; j++) {
              ...
              lColor = colorManager.getColor((int) R, (int) G, (int) B);
